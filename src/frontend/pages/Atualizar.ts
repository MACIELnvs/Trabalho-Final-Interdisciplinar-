import {
    ...
    listarCartas,
    removerCarta
} from "../service/frontService.js";

import type { CartaDTO } from "../tipos/CartasDTO.js";

let bancoCartas: CartaDTO[] = [];

export async function renderAtualizar(container: HTMLElement): Promise<void> {
    container.innerHTML = `
...
function configurarEventosFormulario(): void {
    const inputId = document.getElementById("editId") as HTMLInputElement | null;
    const inputNome = document.getElementById("editNome") as HTMLInputElement | null;
    const inputFrameType = document.getElementById("editFrameType") as HTMLInputElement | null;
...
function configurarPesquisa(): void {
    const inputPesquisa = document.getElementById("inPesquisarAddRem") as HTMLInputElement | null;
    const btnLimparPesquisa = document.getElementById("btnLimparPesquisaAddRem") as HTMLButtonElement | null;
...
async function carregarERenderizarCartas(): Promise<void> {
...
function renderizarCartasAtualizar(cartas: CartaDTO[]): void {
...
function preencherFormulario(carta: CartaDTO): void {
    const inputId = document.getElementById("editId") as HTMLInputElement | null;
    const inputNome = document.getElementById("editNome") as HTMLInputElement | null;
    const inputFrameType = document.getElementById("editFrameType") as HTMLInputElement | null;

    if (inputId) inputId.value = String(carta.id);
    if (inputNome) inputNome.value = carta.nome;
    if (inputFrameType) inputFrameType.value = carta.frameType || "monster";
}
...
function limparFormulario(): void {
    ...
    if (inputFrameType) inputFrameType.value = "";
}

function obterId(inputId: HTMLInputElement | null): number | null {
...
function obterDadosFormulario(
    ...
    inputNome: HTMLInputElement | null,
    inputFrameType: HTMLInputElement | null
): { id: number; nome: string; frameType: string } | null {
    const id = obterId(inputId);
...
function mostrarMensagem(texto: string, tipo: "sucesso" | "erro" | "neutro"): void {
    const mensagem = document.getElementById("mensagem-form") as HTMLParagraphElement | null;
...
function atualizarStatus(texto: string): void {
    const status = document.getElementById("status-cartas-addrem") as HTMLParagraphElement | null;
